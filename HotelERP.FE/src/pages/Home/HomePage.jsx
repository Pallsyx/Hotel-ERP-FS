...
useEffect(() => {
    ...
    // Gọi API lấy điểm đến
    setLoadingAttractions(true);
    attractionApi.getAll().then((res) => {
        // Chỉ lấy tối đa 3 điểm đến nổi bật nhất
        const activeAttractions = res.data ? res.data.filter(a => a.status !== 'INACTIVE').slice(0, 3) : [];
        setAttractions(activeAttractions);
    }).catch(err => {
        console.error("Lỗi khi tải điểm đến:", err);
    }).finally(() => {
        ...
        <section className="bg-[#262b3f] py-16 px-8 border-t border-white/5">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {loadingAttractions ? (
                        <div className="col-span-3 text-center text-white">Đang tải dữ liệu...</div>
                    ) : attractions.length === 0 ? (
                        <div className="col-span-3 text-center text-white">Chưa có điểm đến nào.</div>
                    ) : (
                        attractions.map(attraction => (
                            <div key={attraction.id} onClick={() => navigate('/attractions')} className="bg-white rounded-xl overflow-hidden shadow-lg transition-transform hover:-translate-y-2 cursor-pointer group flex flex-col">
                                <div className="h-56 overflow-hidden relative">
                                    <img src={attraction.imageUrl || 'https://via.placeholder.com/800x600'} alt={attraction.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                                    <div className="absolute inset-0 bg-black/10 group-hover:bg-transparent transition-colors"></div>
                                    {attraction.type && (
                                        <div className="absolute top-4 left-4 bg-white/90 text-[#b4976c] text-xs font-bold px-3 py-1 rounded-full shadow-sm">
                                            {attraction.type}
                                        </div>
                                    )}
                                </div>
                                <div className="p-6 flex-1 flex flex-col justify-center items-center">
                                    <h3 className="text-xl font-serif text-[#262b3f] mb-4 text-center leading-snug">{attraction.name}</h3>
                ...
                <div className="text-center mt-12">
                    <button onClick={() => navigate('/attractions')} className="border border-white text-white hover:bg-white hover:text-[#262b3f] transition-colors duration-300 px-8 py-3 rounded-full text-sm tracking-widest uppercase font-medium">
