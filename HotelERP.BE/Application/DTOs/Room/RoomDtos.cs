...
public record ReportDamageRequest(int RoomId, int? BookingId, string ItemName, string Description, decimal Cost, string Reason, IFormFile? EvidenceImage);
public record DamageReportResponseDto(int Id, string ItemName, string Description, decimal Cost, string? EvidenceImageUrl, DateTime ReportedAt);