...
public class MomoPaymentService : IMomoPaymentService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var dto = new PaymentStatusResponseDto
        {
            ...
            ReferenceType = metadata?.Type ?? (payment.InvoiceId.HasValue ? "INVOICE" : string.Empty),
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogWarning(
                "MoMo IPN signature invalid for orderId={OrderId}, requestId={RequestId}",
                request.OrderId,
                request.RequestId);
            ...
            return ApiResult<object>.Fail(
                ...
                "Chữ ký IPN của MoMo không hợp lệ.");
        }
        ...
        if (payment is null)
        {
            _logger.LogWarning("MoMo IPN payment not found for orderId={OrderId}", request.OrderId);
        ...
        if (Normalize(payment.Status) == "SUCCESS")
        {
            return ApiResult<object>.Ok(
                new { paymentId = payment.Id, status = payment.Status },
                "Payment đã được xử lý trước đó.",
                "PAYMENT_ALREADY_PROCESSED");
        }

        var metadata = ParseMetadata(payment.ProviderResponse) ?? new MomoPaymentMetadata();
        ...
        metadata.RawIpn = JsonSerializer.Serialize(request, JsonOptions);
        ...
        if (request.ResultCode == 0)
        {
            ...
            if (payment.InvoiceId.HasValue)
            {
                var finalizeResult = await _invoiceService.FinalizeAsync(
                    ...
                    new FinalizeInvoiceRequestDto
                    {
                        ...
                        Note = $"MoMo IPN success. transId={request.TransId}; payType={request.PayType}; msg={request.Message}"
                    },
                    null,
                    cancellationToken);
            ...
            return ApiResult<object>.Ok(
                new { paymentId = payment.Id, orderId = request.OrderId, transId = request.TransId, status = payment.Status },
                "MoMo IPN xử lý thành công.",
                "MOMO_IPN_SUCCESS");
        }
        ...
        return ApiResult<object>.Ok(
            new { paymentId = payment.Id, orderId = request.OrderId, resultCode = request.ResultCode, status = payment.Status },
            "MoMo báo giao dịch thất bại.",
            "MOMO_IPN_FAILED");
    }
